...
function eachCursor(customStore: UseStore, callback: (cursor: IDBCursorWithValue) => void): Promise<void>
{
...
* @param customStore Method to get a custom store. Use with caution (see the docs).
*/
export async function keys<KeyType extends IDBValidKey>(customStore = defaultGetStore()): Promise<KeyType[]>
{
	const items: KeyType[] = [];
	await eachCursor(customStore, (cursor) => items.push(cursor.key as KeyType));
	return items;
}
...
* @param customStore Method to get a custom store. Use with caution (see the docs).
*/
export async function values<T = any>(customStore = defaultGetStore()): Promise<T[]>
{
	const items: T[] = [];
...
* @param customStore Method to get a custom store. Use with caution (see the docs).
*/
export async function entries<KeyType extends IDBValidKey, ValueType = any>(customStore = defaultGetStore()): Promise<[KeyType, ValueType][]>
{
	const items: [KeyType, ValueType][] = [];
	await eachCursor(customStore, (cursor) => items.push([cursor.key as KeyType, cursor.value]));
	return items;
}